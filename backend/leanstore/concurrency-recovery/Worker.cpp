...
// -------------------------------------------------------------------------------------
void Worker::startTX(TX_MODE next_tx_type, TX_ISOLATION_LEVEL next_tx_isolation_level, bool read_only, unsigned int namespace_id)
{
   ...
   active_tx.namespace_id = namespace_id;
   if (FLAGS_wal) {
  ...
  if (activeTX().isDurable()) {
    {
      ...
      entry.namespace_id = active_tx.namespace_id;
